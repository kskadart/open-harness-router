...
- This session and every subagent reach the API THROUGH the router being
  reconfigured. `routing.yaml` and `.env` are read once at startup, so a
  provider whose key variable is unset or whose CA bundle is missing does
  not degrade the router -- it stops it from starting at all, and launchd's
  KeepAlive then crash-loops it.
...
- The restart step begins with `uname -s`, BEFORE any command that could
  ...
  ready-to-paste systemd block (step 9b).
...
- One gateway is ONE provider block, however many models it serves: models
  are `rules:` entries (first match wins, case-sensitive) and the provider's
  `max_tokens_limit` / `context_window` are defaults a rule overrides --
  `routing.example.yaml` carries the shape.
- The `/model` picker and the agent frontmatter are fed from `client_models`
  on the rule, regenerated by `make sync-client-config` (step 12). That
  client config also disables Claude Code's own window enforcement, so
  `context_window` on every reachable openai-translate provider stops being
  optional -- step 6b.
...
- Secret: `Authorization: Bearer <...>` or `x-api-key: <...>`. `x-api-key`
  ...
  Bearer and `extra_headers` may not carry auth -> stop and ask.
...
The variable is inert until the restart. To undo:
...
| 2 | `BUNDLE_UNUSABLE` | nothing was probed -- fix the bundle |
...
What each code means is the `Verdict` enum in `src/cli/tls_probe.py`; 1 is
never a verdict.

The probe verdict is authoritative: it uses exactly the trust store the
router will use. Never write into `proxy-ca/` (the forward-proxy MITM CA);
files found there are copied, not deleted.
...
`min(max(client, 100), max_tokens_limit)`, silently, where
`max_tokens_limit` is the rule's value when it sets one and the provider's
otherwise. Too low -> reasoning models return empty `content` with
`stop_reason: max_tokens`; too high -> a vLLM-style upstream answers 400.
Ladder:
...
of the model that was probed at that value. Record the source of each number
and the probe outcome in a YAML comment next to it.

#### 6b. `context_window` -- the deployment's total window

Field semantics and the startup invariant: `routing.example.yaml`.
...
3. The upstream's own error text on the oversized request of 6a probe 2:
   ...
   prompt_too_long` shape. If the upstream answers an overflow with a 5xx instead
...
Rules: English comments; no dead `timeout_s`;
...
forbidden on passthrough -- there the alias is what the client sends -- and so are `max_tokens_limit` and
...
(its value is the id). Startup validation rejects an entry the rule's own
match does not accept, one an earlier rule captures, and one listed twice.
...
Do NOT run the script here: it drives `launchctl` and exits 64, printing
this recipe. Run it instead, as ONE Bash call, with YOUR unit name:

```sh
systemctl --user restart YOUR-UNIT
timeout 40 sh -c 'until curl -sf http://127.0.0.1:8787/health; do sleep 1; done' \
  || echo "FAIL: no /health in 40s; journalctl --user -u YOUR-UNIT -n 50" >&2
```

This block does not compare pids or check `--expect-provider`; read the
`/health` body it prints. There is NO automatic rollback here: on `FAIL:`
restore the backup by hand and run the block again:
...
| `open-harness-router: provider 'x': CA bundle not found` | `ca_bundle` is relative to `certs_dir` (`certs/`, from `ROUTER_CERTS_DIR`); the message names the resolved path. `cli.tls_probe match` prints the `cat` line that creates the bundle |
...
| empty final message from a subagent, or `output_tokens: 1` with `stop_reason: end_turn` | check the router log for the `empty_completion` warning: the upstream closed the turn with no text and no tool call. The router forwards in-conversation system messages as user text on the chat flavor because some open-model chat templates stop on a trailing system message; if the warning still appears, capture the wire body and compare what the upstream received |
