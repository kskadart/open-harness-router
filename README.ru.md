...
> macOS, но сам роутер и его CLI-помощники (`cli.validate_routing`,
> `cli.tls_probe`, `cli.sync_client_config`) — это обычный Python без
> платформозависимого кода: набор тестов гоняется на Ubuntu в CI
> (`.github/workflows/ci.yml`, `runs-on: ubuntu-latest`), а README покрывает
> запуск сервиса под systemd на Linux. Что относится только к macOS — это шаг
> перезапуска в скилле add-provider: его `restart_router.sh` общается с
> launchd и на любой другой ОС завершается кодом 64 с подсказкой про systemd,
> а путь журнала по умолчанию в скилле предполагает `~/Library/Logs`. На
> Linux перезапускайте сервис вручную (`systemctl --user restart ...` со
> своим реальным именем юнита) и укажите `OHR_ERR_LOG` на свой собственный
> журнал. Windows не тестировался.
...
- `cli.tls_probe match` считает отпечатки переданных PEM-файлов и сравнивает
  ...
  сертификата — код 2. Сам он файлы никогда не пишет. На stdout уходят
  только `REUSE`, команда `cat` и инвентарь сертификатов; каждая строка
  `WARNING:` идёт в stderr, а когда переиспользуемый бандл шире входа,
  лишние сертификаты, которые он несёт (subject, notAfter, sha256),
  называются в stderr — новый провайдер будет им тоже доверять. Код 1 не
  использует ни одна из подкоманд: это статус, который Python даёт
  неперехваченному исключению, так что падение нельзя прочитать как
  результат.
...
не хватает, чтобы отличить их друг от друга. Скрипту нужен реальный
launchd-label: по умолчанию это заглушка `com.example.open-harness-router`,
а реальное имя берётся по порядку: переменная `OHR_LAUNCHD_LABEL`, затем
игнорируемый git файл `.launchd-label` в корне репозитория (его первая
непустая строка), иначе остаётся заглушка; если не задано ни то ни другое,
запуск без аргументов завершается кодом 65 с подсказкой, называющей этот
файл. По таймауту — или по ошибке старта в err.log (строка
`open-harness-router: ...` или traceback), всё ещё только когда передан
`--routing-backup` — скрипт копирует файл из `--routing-backup` обратно
поверх `routing.yaml`, перезапускает сервис снова и завершается с кодом 1
(0 — здоров на новой конфигурации, 2 — лежит, пустой `.launchd-label` или
откат невозможен, 64 — не macOS, 65 — сервис не загружен в launchd-домен
пользователя). Этот шаг перезапуска работает только под launchd: на любой
другой ОС скрипт завершается кодом 64 с подсказкой про systemd, поэтому на
Linux перезапускайте сервис вручную и укажите `OHR_ERR_LOG` на свой журнал.
`OHR_HEALTH_TIMEOUT_S` ограничивает ожидание в секундах
реального времени, а не числом опросов. Запускать её должна только сессия верхнего уровня и
только когда не работает ни один субагент: во время вызова Bash у самой
сессии нет открытого стрима, а у работающего субагента он есть, и
...
- `context_window` — опционально, полный контекст развёртывания в токенах
  (промпт плюс ответ). Должен быть больше `max_tokens_limit` плюс
  `CONTEXT_WINDOW_RESERVE_TOKENS` (512) плюс `MIN_USEFUL_COMPLETION_TOKENS`
  (4096) — тот же нижний предел, что и в предварительной проверке, — а
  явное значение на `passthrough` — ошибка при старте. Не задан — нет ни
  оценки токенов, ни предварительной проверки, запросы уходят наверх без
  изменений. Задан — включается защита от переполнения, описанная в
  «Контекстное окно и подсчёт токенов» ниже. Как и `max_tokens_limit`, это
  значение по умолчанию, которое правило может переопределить для
  конкретной модели.
...
должно быть больше `max_tokens_limit` плюс `CONTEXT_WINDOW_RESERVE_TOKENS`
(512) плюс `MIN_USEFUL_COMPLETION_TOKENS` (4096), иначе один только предел
на ответ вместе с резервом заполняет окно и любой промпт отвергался бы.
Не задано — нет ни оценки, ни предварительной проверки: запрос уходит наверх
...
- `openai-translate` — оценивается локально
  ...
  base64 не учитываются, а закэшированные элементы рассуждений считаются
  по длине их зашифрованного содержимого с запасом в большую сторону. Это символьная
  эвристика (ASCII- и не-ASCII-символы по разным ставкам, с отдельной
  корзиной CJK по одному символу на токен — символы и пунктуация CJK,
  хирагана, катакана, иероглифы хань, хангыль и полноширинные формы — плюс
  накладные расходы на сообщение и на инструмент), а не токенизатор: парк
  смешивает токенизаторы, а защите нужна лишь безопасная верхняя оценка.
  Откалибровано 2026-09-04 по `usage.input_tokens` двух моделей парка на
  трёх телах каждая: оценка на всех образцах оказывалась выше реального
  счёта в 1.10-1.33 раза, то есть она намеренно немного завышает.
...
- `modelPicker.options` — по одной строке на каждую предлагаемую модель, с
  ...
  `prefix`/`contains`/`regex` без `client_models` пропускается с
  предупреждением, называющим правило, потому что его значение match — это
  шаблон, а не идентификатор модели (`gpt-`, отправленный наверх дословно,
  даёт 404 у вендора); запуск падает, только если вообще нельзя объявить
  ни одной модели. Правила, обслуживаемые провайдером `passthrough`,
  пропускаются — нативные идентификаторы `claude-*` уже есть в пикере.
...
(временный файл плюс `os.replace`), сохраняет прежний режим доступа к файлу и
оставляет предыдущую версию нетронутой при неудачной записи; неразбираемый
файл вызывает отказ, а не перезапись. `--check` сообщает, соответствует ли
файл `routing.yaml`, ничего
