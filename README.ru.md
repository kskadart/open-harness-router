...
- `cli.tls_probe match` считает отпечатки переданных PEM-файлов и сравнивает
  ...
  сертификата — код 2. Сам он файлы никогда не пишет. Каждая строка
  `WARNING:` идёт в stderr, включая сертификаты (subject, notAfter,
  sha256), которые переиспользуемый бандл несёт сверх входа, — новый
  провайдер будет им тоже доверять. Код 1 не использует ни одна из
  подкоманд: это статус, который Python даёт неперехваченному исключению,
  так что падение нельзя прочитать как результат.
- `cli.tls_probe probe` выполняет одно TLS-рукопожатие ровно с тем
  ...
  вердикт. Именно это отделяет несовпадение имени хоста от сломанной
  цепочки: `CHAIN_OK_HOSTNAME_OK` (код 0) не требует ничего, кроме
  `ca_bundle`, `BUNDLE_UNUSABLE` (2) означает, что бандл из `--cafile` не
  удалось загрузить и рукопожатие не выполнялось,
...
`--routing-backup` — скрипт сохраняет отвергнутую конфигурацию как
`routing.yaml.failed-<epoch>`, копирует файл из `--routing-backup` обратно
поверх `routing.yaml`, перезапускает сервис снова и завершается с кодом 1
(0 — здоров на новой конфигурации, 2 — плохие аргументы, пустой
`.launchd-label`, сервис лежит или откат невозможен, 64 — не macOS,
65 — сервис не загружен в launchd-домен пользователя). Этот шаг перезапуска
работает только под launchd: на любой другой ОС скрипт завершается кодом 64
с подсказкой про systemd, поэтому на Linux перезапускайте сервис вручную и
укажите `OHR_ERR_LOG` на свой журнал.
`OHR_HEALTH_TIMEOUT_S` ограничивает ожидание в секундах реального времени.
Запускать её должна только сессия верхнего уровня и только когда не работает
ни один субагент: во время вызова Bash у самой сессии нет открытого стрима,
а у работающего субагента он есть, и `kickstart -k` его обрывает.
...
(512) плюс `MIN_USEFUL_COMPLETION_TOKENS` (4096). Не задано — нет ни оценки,
ни предварительной проверки: запрос уходит наверх ровно так же, как раньше.
...
и клиентские настройки»), поэтому с этого момента предварительная проверка
роутера — единственная защита: у модели `openai-translate` без эффективного
`context_window` (ни в правиле, ни у провайдера) её нет вовсе, и именно
поэтому `cli.sync_client_config` отказывается объявлять такую модель.
...
апстрима на завышенном тестовом запросе (OpenAI-совместимая 400 про длину
...
- `openai-translate` — оценивается локально
  ...
  проходит через тот же сборщик запроса, что и `/v1/messages`, то есть это
  символьная эвристика ровно по сконвертированному телу, уходящему в сеть,
  откалиброванная 2026-09-04 по `usage.input_tokens` двух моделей парка с
  завышением в 1.10-1.33 раза.
...
- если по оценке промпт оставляет от этого бюджета меньше
  `MIN_USEFUL_COMPLETION_TOKENS` (4096, `src/const.py`), запрос отвергается
  ...
  prompt_too_long)` и событие лога `context_window_reject` (`provider`,
...
- `modelPicker.options` — по одной строке на каждую предлагаемую модель, с
  ...
  провайдера, объявляют собственные лимиты). Правило
  `prefix`/`contains`/`regex` без `client_models` пропускается с
  предупреждением, называющим правило, а запуск падает, только если вообще
  нельзя объявить ни одной модели. Правила, обслуживаемые провайдером
  `passthrough`, пропускаются — нативные идентификаторы `claude-*` уже есть
  в пикере.
- `env.CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT` = `"1"` — без
  ...
  `claude-*` сохраняют обычное сжатие (см. «Контекстное окно и подсчёт
  токенов»).
...
файл `routing.yaml`, ничего не записывая (код возврата 3, если не
соответствует), а `--settings-path` направляет запись в другой файл.
