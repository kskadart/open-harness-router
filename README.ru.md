...
Эта же уверенность включает одну first-party-возможность протокола, которую
роутеру приходится обслуживать самому: треды сообщений (Claude Code
v2.1.278+, бета `message-threads-2026-08-12`). Первый запрос хода несёт весь
диалог с `thread: {"type": "create"}`; последующие шаги с результатами
инструментов несут только дельту -- блок атрибуции вместо всего `system`,
новые результаты инструментов, без прежних сообщений и без tools -- с
`thread: {"type": "continue", "previous_message_id": ...}`, рассчитывая, что
апстрим хранит остальное под id предыдущего ответа. Passthrough передаёт и
то и другое как есть: тред хранит Anthropic. Для `openai-translate` тред
хранит роутер (`src/services/thread_store.py`): каждый ответ, выданный на
запрос с полем `thread`, запоминается вместе с породившим его запросом, а
продолжение перед трансляцией собирается в тот полный запрос, который
клиент прислал бы без тредов -- сохранённый системный промпт с блоком
атрибуции, обновлённым из дельты, сохранённые сообщения, записанный ход
ассистента, сообщения дельты, сохранённые tools. Хранилище держит последние
64 ответа в течение часа (`THREAD_STORE_*` в `src/const.py`; клиент
продолжает только от своего последнего). Продолжение, которое собрать нечем
-- после рестарта, вытеснения или по TTL, -- получает `400
invalid_request_error` без обращения к апстриму, и CLI повторяет тот же шаг
полным запросом `create` (измерено на 2.1.278, формулировка ошибки роли не
играет). Уйди такая дельта в апстрим как есть, тот отвечал бы на запрос без
системного промпта и без вопроса: шлюз, подставляющий собственный промпт по
умолчанию, здоровается посреди цикла инструментов. В логе каждый собранный
запрос виден событием `thread_resumed`, каждый промах --
`thread_continue_rejected` с полем `reason`; отказ несёт заголовок
`x-ohr-capability-rejected: thread_continue` и попадает в ленту дашборда с
пометкой `rejected`, а не как ошибка. За кастомным `ANTHROPIC_BASE_URL` CLI
тредов не шлёт.
...
- `passthrough` работает как reverse-proxy на Anthropic-совместимый апстрим,
  ...
  (`upstream_model` имеет смысл только для `openai-translate`). Ответы
  передаются как получены, и потоковые, и обычные: байты тела и
  `content-encoding` апстрима доходят до клиента нетронутыми, так что
  кодировку, о которой клиент договорился (Brotli у Claude Code),
  распаковывает сам клиент. Распаковка в роутере оставляла бы тело
  закодированным всякий раз, когда апстрим выбирает кодек, которого у
  роутера нет, -- именно так вердикты классификатора auto-режима
  (нестримовые ответы) превращались в нечитаемый JSON. Query-string
...
- `openai-translate` переводит запрос в OpenAI ChatCompletions (свой
  ...
  ответ. Отложенные инструменты скрыты так же, как на Claude API:
  инструмент с пометкой `defer_loading` (поиск по MCP-инструментам в Claude
  Code) уходит в апстрим только после того, как блок `tool_addition` в
  диалоге на него сослался, а запись `DeferredToolPlaceholder` не уходит
  никогда -- транслируемая модель видит те инструменты, которые
  действительно может вызвать, а не все MCP-схемы, и не может вызвать
  инструмент, который Claude Code не загрузил. Ссылка на инструмент,
  которого в запросе нет, пишется в лог событием `tool_addition_unresolved`
  и пропускается. Транслируемый поток поддерживается живым так же, как
  Claude API поддерживает свои: если видимых событий не было дольше
  `STREAM_KEEPALIVE_INTERVAL_S` (10 с, `src/const.py`) -- рассуждающий
  апстрим думает, шлюз держит запрос в очереди -- роутер отправляет клиенту
  событие `ping`, и сторожевые таймеры Claude Code на молчащий поток не
  обрывают запрос с повторной отправкой всего диалога. Поток, которому
  понадобились пинги, пишется в лог событием `stream_keepalive` с их числом
  и самой длинной паузой.
...
- каждый маршрутизируемый запрос `/v1/messages` и
  ...
  то же чтение работает и для транслированных ответов. Тело в gzip или
  deflate, потоковое или нет (так `api.anthropic.com` отвечает клиенту,
  который это принимает; passthrough передаёт его сжатым),
  распаковывается только для чтения; тело в кодировке, которую stdlib не
  умеет (`br`, `zstd` -- её `api.anthropic.com` выбирает для нестримовых
  запросов Claude Code), уходит как есть и токенов не даёт. `400`, которым
  транслирующий провайдер отвечает
  намеренно (продолжение треда сообщений, собрать которое нечем, см.
  «Forward-proxy» в «Режимах работы»), попадает в ленту с полем `rejected`
  и считается запросом, а не ошибкой;
...
src/
  ...
  version.py         версия приложения из pyproject.toml (его правит cli.release)
  const.py           constants
  ...
  services/          headers, reasoning-context cache, хранилище тредов сообщений, token estimation, монитор дашборда
