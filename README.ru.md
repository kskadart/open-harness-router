...
- `passthrough` работает как reverse-proxy на Anthropic-совместимый апстрим,
  ...
  (`upstream_model` имеет смысл только для `openai-translate`). Поддерживает
...
стороннего вендора за его собственный ключ (два режима авторизации сквозного
...
Ограничение самого CLI: пока `ANTHROPIC_BASE_URL` смотрит не на
`api.anthropic.com`, Remote Control недоступен (по состоянию на v2.1.196).
Ограничение касается только режима reverse-proxy, forward-proxy обходит его,
не теряя маршрутизации, см. ниже.
...
(`ANTHROPIC_BASE_URL` смотрит на роутер), а как HTTP-прокси (клиент ходит
через `HTTPS_PROXY`). Режим появился из-за описанного выше ограничения CLI:
Claude Code выключает Remote Control всякий раз, когда `ANTHROPIC_BASE_URL`
смотрит не на `api.anthropic.com` (начиная с CLI 2.1.196), но переменную
`HTTPS_PROXY` эта функция переживает. Здесь клиент стартует с нетронутым
